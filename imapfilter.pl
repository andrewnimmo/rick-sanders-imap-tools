...
# $Header: /mhub4/sources/imap-tools/imapfilter.pl,v 1.50 2016/03/10 14:35:51 rick Exp $
...
sub processArgs {
   ...
   ($host,$user,$pwd,$method) = split(/\//, $opt_S);
   if ( uc( $method ) eq 'CRAM-MD5' ) {
      $method = 'CRAM-MD5';
   }  elsif ( $host =~ /CRAM-MD5/i ) {
      $method = 'CRAM-MD5';
      $host =~ s/\/CRAM-MD5//i;
   }
   Log("Using CRAM-MD5 login method") if $method eq 'CRAM-MD5';
...
sub usage {
   ...
   print STDOUT "    To use CRAM-MD5 for logins add /CRAM-MD5 like this  -S host/user/pwd/CRAM-MD5\n\n";
   exit;
