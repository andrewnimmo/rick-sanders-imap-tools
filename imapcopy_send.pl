...
#################################################################
   ...
   $notify_file = 'imapcopy_notify.admin';
   print "Looking for $notify_file\n";

   if ( -e $notify_file ) {
      print "Found $notify_file\n";
      open(N, "<$notify_file") or print "Error opening $notify_file: $!\n";
      chomp($notify = <N>);
      print "notify line is $notify\n";
      ($smtp_server,$admin) = split(/:/, $notify);
      print "smtp_server = $smtp_server\n";
      print "admin addr  = $admin\n";
      send_mail( $smtp_server, $admin, "$user migrated", $message );
      close N;
   } else {
      print "$notify_file was not found\n";
   }
   ...
   connectToHost($sourceHost, \$src)   or exit 1;
   login($sourceUser,$sourcePwd, $sourceHost, $src, $srcMethod) or exit 1;
   namespace( $src, \$srcPrefix, \$srcDelim, $opt_x );

   connectToHost( $destHost, \$dst ) or exit 1;
   login( $destUser,$destPwd, $destHost, $dst, $dstMethod ) or exit 1;
   ...
   if ( $ENV{'HTTP_CONNECTION'} ) {
      ...
      notify_user( $sourceUser, $src, \@too_large, \@mbx_errors, 1 );
      notify_user( $destUser,   $dst, \@too_large_1, \@mbx_errors, 0 );
   }
...
sub init {
   ...
   Log("Copied msgs will be marked on the destinationas as SEEN") if $mark_as_seen;
...
my $method = shift;
   ...
   if ( lc( $host ) eq 'imap.gmail.com:993' ) {
      ...
      login_plain( $user, $user, $pwd, $conn ) or exit 1;
      return 1;
   }
   ...
   if ( $user =~ /(.+):(.+)/ ) {
      ...
      $sourceUser  = $1;
      $authuser    = $2;
      login_plain( $sourceUser, $authuser, $pwd, $conn ) or exit 1;
      return 1;
   }  
...
my @mbxs;
   ...
   if ( $mbxList ) {
      ...
      foreach $mbx ( split(/\Q$mbx_delimiter/, $mbxList) ) {
...
my @mbxs;
   ...
   while ( 1 ) {
        &readResponse ($conn);
        if ( $response =~ /\{(.+)\}$/ ) {
           #  The name of the mailbox is on the next line
           &readResponse ($conn);
   ...
   for $i (0 .. $#response) {
        $response[$i] =~ s/\s+/ /;
        if ( $response[$i] =~ /\{(.+)\}$/ ) {
           #  Mailbox name is on the next line
           $mbx = $response[$i+1];
        } elsif ( $response[$i] =~ /"$/ ) {
           $response[$i] =~ /\* LIST \((.*)\) "(.+)" "(.+)"/i;
           $mbx = $3;
   ...
   for $i (0 .. $#response) {
	last if $response[$i] =~ /^1 OK FETCH complete/i;
        ...
        if ( $response[$i] =~ /^\)/ or ( $response[$i] =~ /\)\)$/ ) ) {
           ...
           if ( $skip_deleted and $flags =~ /\\deleted/i ) {
              Log("   Skipping msgnum $msgnum because it is marked for Delete") if $debug;
              next;
           }
...
for $num (@msgList) {
     ...
     sendCommand ( $conn, "1 FETCH $num (uid flags internaldate RFC822.SIZE body.peek[header.fields (From Message-Id Subject Date)])");
...
sub usage {
   ...
   print STDOUT "          -m mailbox list (eg \"Inbox, Drafts, Notes\". (Or \"mbx1|mbx2|...mbxn\"). Default is all mailboxes)\n";
   ...
   print STDOUT "          -N Copy Gmail '[Gmail]/All Mail' mailbox (normally skipped)\n";
   print STDOUT "          -v Mark copied messages as SEEN (READ)\n";
   print STDOUT "          -P Don't copy source msgs which are marked for Delete\n";
   exit;
...
sub processArgs {
   ...
   if ( !getopts( "dS:D:L:m:hIp:M:rqx:y:e:Rt:Tia:b:X:vPA:UB:E:uHzZ:j:g:CsnWwGF:cK:Y:kfQJ:lo:OVN" ) ) {
      usage();
   }
   if ( $opt_S =~ /\\/ ) {
...
my $substChar = '_';
   ...
   unless ( $srcmbx =~ /\[Gmail]\// ) {
      if ( ($srcmbx =~ /[$dstDelim]/) and ($dstDelim ne $srcDelim) ) {
         ...
         $srcmbx =~ s^[$dstDelim]^$substChar^g unless $opt_G;
      }
   }
...
my $msgcount=0;
   ...
   for $i (0 .. $#response) {
       ...
       if ( $response[$i] =~ /^\)/  or ( $response[$i] =~ /\)\)$/ ) ) {
          ...
          $$msgids{"$msgid"} = $msgnum;
          $msgid = $from = $subject = $header_date ='';
       }
   }
...
my $rc;
   ...
   while (1) {
        ...
        if ( $response =~ /^1 OK/i ) {
	   $rc = 1;
	   Log("   Marked $msgid for delete") if $debug;
	   last;
	}
...
";
   ...
   $notify_file = $0;
   $notify_file =~ s/imapcopy.pl/imapcopy_notify.admin/;

   if ( -e $notify_file ) {
      Log("Notify the administrator as well");
      open(N, "<$notify_file") or Log("Error opening $notify_file: $!");
      chomp($notify = <N>);
      ($smtp_server,$admin) = split(/:/, $notify);
      send_mail( $smtp_server, $admin, "$user migrated", $message );
      close N;
...
my $copied = 0;
        ...
        Log("   Copying $msgcount messages in $srcmbx mailbox") if $debug;
        if ( $msgcount == 0 ) {
        ...
        foreach $_ ( @msgs ) {
           ...
           if ( $copied/100 == int($copied/100)) {
              Log("   Copied $copied messages so far") if $debug;
           }
...
my @summary;
   ...
   foreach $srcmbx ( @$mbxs ) {
      ...
      exit 1 if $$ ne $parent_pid;    # Don't let a child try to launch another child
...
my $msgid;
   ...
   #  Construct a messageID from the date, subject, and from fields
...
my $body        = shift;
   ...
   ($username,$domain) = split(/\@/, $to);
   $from = "imapcopy\@$domain";
   print "from $from\n";
   ...
   print "1 sreply $sreply\n";

   send SERVER, "HELO hopehealthsystems.com\r\n", 0;
   recv SERVER, $sreply, 512, 0;
   print "2 sreply $sreply\n";
  
   send SERVER, "MAIL From:<$from>\r\n", 0;
   recv SERVER, $sreply, 512, 0;
   print "3 sreply $sreply\n";

   @tos = split(/,/, $to);
   foreach $to ( @tos) {
      send SERVER, "RCPT To:<$to>\r\n", 0;
      recv SERVER, $sreply, 512, 0;
      print "4 sreply $sreply\n";
   }

   send SERVER, "DATA\r\n", 0;
   recv SERVER, $sreply, 512, 0;
   print "5 sreply $sreply\n";

   send SERVER, "Subject: $subject\r\n", 0;
   send SERVER, "To: $to\n", 0;
   send SERVER, "From: $from\n", 0;

   send SERVER, "\r\n$body\r\n", 0;
   send SERVER, ".\r\n", 0;
   recv SERVER, $sreply, 512, 0;
   print "6 sreply $sreply\n";

   print "Sendmail status $sreply\n";

   send SERVER, "QUIT\r\n", 0;
   recv SERVER, $sreply, 512, 0;
   print "7 sreply $sreply\n";
