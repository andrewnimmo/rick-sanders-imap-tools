...
# $Header: /mhub4/sources/imap-tools/imapcopy.cgi,v 1.14 2016/02/08 17:09:39 rick Exp $
...
sub list_folders {
  ...
  Log("This is list_folders.") if $debug;
...
sub list_imap_folders {
   ...
   foreach $mbx ( @mbxs ) {
      ...
      commafy( \$msgcount );
      commafy( \$size );
      print STDOUT "<tr><td>$mbx<td>$msgcount<td>$size<br>" unless $mbx eq '';
      $total_msgs += $msgcount;
      $total_bytes += $size;
   }
   commafy( \$total_msgs );
   commafy( \$total_bytes );
   print STDOUT "<tr><td><b>Totals<td>$total_msgs<td>$total_bytes<br>";
   print STDOUT "</table>";
